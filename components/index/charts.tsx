...
function separateInnerAndOuterPie(
  ...
  if (data.length > n) {
    return [data.slice(0, n).map(trnasfer), data.slice(n).map(trnasfer)];
  }

  return [data.map(trnasfer), []];
}

function InnerOuterPie(
  props: Combine<
    ...
    <InnerOuterPie
      ...
      option={{
        tooltip: {
          ...
          formatter: (
            item: any,
          ) => `<div style="max-width:15em; white-space: normal; word-break: break-all;">
...
function DomainChart() {
  ...
  return <InnerOuterPie loading={loading} data={data} title="域名统计图" />;
}
...
export function Charts() {
  return (
    <Flex direction="TB" mainAxis="space-around" fullWidth>
      {charts.map((item) => (
        <Card key={item.name} style={{ background: "#fcfcfc" }}>
          {item.chart}
        </Card>
      ))}
    </Flex>
