...
async function getArchDesp() {
  ...
  try {
    if (!globalThis.archDesp || globalThis.archDespDatetime + expired < now) {
      const resp = await fetch("https://proxy.ohyee.cc/raw.githubusercontent.com/zh-blogs/blog-architecture/main/arch.json");
...
export default wrapper<typeof getArchCharts>(
  async (args, req) => {
    if (req.method === "GET") {
