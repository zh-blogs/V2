...
async function getInfo(token: string): Promise<UserInfo> {
  const resp = await fetch(
    "https://proxy.ohyee.cc/https://api.github.com/user",
    {
      headers: {
  ...
  const resp2 = await fetch(
    `https://proxy.ohyee.cc/https://api.github.com/orgs/zh-blogs/members/${info.login}`,
  );
...
export default async function github_login(
  ...
  if (req.method === "GET") {
    if (!!args.code && !!args.state) {
      ...
      res.writeHead(302, {
        Location: args.state,
        "Set-Cookie": `token=${frontToken}; Path=/; Max-Age=${
          60 * 60 * 24 * 7
        }`,
      });
      res.end();
