...
import { DatabaseLoki, newLokiCached } from './wrapper';
import { Log } from '@/utils/log';
import { shouldNumber, shouldString } from '@/utils';
import { v4 as uuid } from 'uuid';

export class LokiJSDB {
  log!: Log;
  DB!: DatabaseLoki;
  ...
  async getUser(params: { token: string }): Promise<Result<UserInfo>> {
    const c = this.DB.getUsersCollection();
  ...
  setUser(params: { token: string; info: UserInfo }) {
    const c = this.DB.getUsersCollection();
    const cSetting = this.DB.getSettingsCollection();
